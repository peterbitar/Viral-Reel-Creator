...
async function approveVariant(variantId: number): Promise<void> {
  ...
  stmt.run("approved", variantId);

  console.log(`[UI] Variant ${variantId} approved (was: ${variant.status})`);
}
...
async function main(): Promise<void> {
  ...
  if (!variantId || isNaN(variantId)) {
    console.error("[UI] Usage: npm run approve -- --variant_id=<id>");
    process.exit(1);
  }
